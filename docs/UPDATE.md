...
# 前端本地热更新: cd web && npm run dev → http://localhost:5173/(代理 /api → 8899)
```
...
用户端前端（`web/`）用 Vite 内容 hash 文件名，**无需手动升版本号**：CI 的 `build-and-push` 自动 `npm run build` 并打进镜像，根路径 `/` 直接服务新产物。本地开发跑 `./scripts/build_web.sh`（单测 + 构建）生成 `web/dist/` 即可本地验证。

管理后台（`server/app/web/admin/`，独立保留的手写静态资源）用缓存破坏（`?v=`）：改 `admin/assets/*` 后必须同步升级 `admin/index.html` 中的 `?v=` 值。
...
**前端部署**：CI 的 `webapp` job 验证构建；`build-and-push` 自动构建 `web/dist/` 并打进镜像（容器内 `/web/dist`，FastAPI 检测到 `index.html` 即在根路径 `/` 服务正式版；`/next/*` 旧灰度地址 301 回 `/`）。本地源码构建（`install.sh` 走 `up -d --build`）若机器有 npm 会自动构建，否则 `/` 返回 JSON 兜底、管理后台 `/admin` 与落地页 `/welcome` 不受影响。
