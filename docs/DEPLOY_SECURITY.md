...
8. **密码策略**：最少 8 位 + 弱密码黑名单 + 不得与用户名相同；首次部署管理员密码弱则拒绝创建（`ALLOW_WEAK_ADMIN_PASSWORD` 调试放行）。
9. **敏感文件 Guard**：同步/上传前扫描文件名与内容（含 PDF/Word/Excel/PPT 提取文本），凭据类硬拦、隐私类告警，方向感知。
10. **审计日志**：登录/文件操作/管理操作/限流锁定/密保重置失败全量记录，含真实客户端 IP。
11. **令牌生命周期**：设备令牌可单独/全部吊销；被禁用账户无法刷新 token。
12. **反枚举**：忘记密码接口对所有用户名返回统一提示；密保重置对不存在用户补一次等量 bcrypt，消除时序差。
...
- Caddy 反向代理部署：把 Caddy 容器 IP 填入 `TRUSTED_PROXIES`（如 `172.18.0.3`），
  这样服务端能拿到真实客户端 IP。
- **切勿把 `8000` 端口直接暴露到公网**（`docker-compose.yml` 的 `8000:8000` 仅供调试）；
