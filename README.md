...
代码改动后的上线流程（改代码 → push → CI 构建镜像 → 服务器拉取重启）、改配置、回滚、迁移新服务器，详见 [docs/UPDATE.md](docs/UPDATE.md)。
...
打开浏览器，访问 `https://files.yourdomain.com`，登录即可浏览文件、与 AI 对话、在线预览。浏览器端默认禁止下载（文件不落公司电脑），确需下载时到设置页开启「临时下载」5 分钟窗口；在线预览带 `no-store`，关页即失。
...
- [x] JWT 认证 + TOTP 双因子
- [x] 设备令牌管理（可吊销）
- [x] 浏览器会话令牌可远程吊销（改/重置密码 bump 版本号，旧会话立即失效）
- [x] 临时下载授权（浏览器默认禁下载，5 分钟窗口，到期自动关闭）
- [x] 在线预览 no-store（关页即失，不进磁盘缓存）
...
- [x] 存储统计 + 磁盘监控
- [x] 清理建议（长期未用文件）
- [x] 回收站（软删除 + 可配置保留期 + 恢复/彻底删除 + 过期自动清理 + 锁存跳出清理 + 批量恢复/删除 + 清空确认词防误操作 + 回收站内只读预览 + 管理后台总览）
