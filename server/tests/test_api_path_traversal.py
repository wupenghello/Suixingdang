...
def test_download_traversal_returns_404(client, make_user):
    token, _uid, _ = make_user()
    client.post("/api/files/download-grant", headers=_h(token), json={"password": "Test1234pass", "minutes": 0})  # 开启临时下载以测路径校验
...
def test_legitimate_subdir_upload_download_not_regressed(client, make_user):
    ...
    client.post("/api/files/download-grant", headers=_h(token), json={"password": "Test1234pass", "minutes": 0})  # 开启临时下载
    r = client.get("/api/files/download",
